# -*- coding: utf-8 -*-
"""Copy of Streamlit CAS to H-Codes 2.0.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1KRi2UPHtA-q5Kku4CX8DEDy0PQ8FTW2o
"""
...
def get_h_codes_from_pubchem(chemical_name):
    ...
    specifically filtering for 'European Chemicals Agency (ECHA) C&L Inventory' source.
    """
    headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36'}
                ...
                if ghs_classification_section:
                    for info_block in ghs_classification_section.get('Information', []):
                        if info_block.get('Name') == 'GHS Hazard Statements' and 'Value' in info_block:
                            # Check if the source is ECHA C&L Inventory
                            is_echa_source = False
                            for reference in info_block.get('Reference', []):
                                if reference.get('Source') == 'European Chemicals Agency (ECHA) C&L Inventory':
                                    is_echa_source = True
                                    break
                            ...
                            if is_echa_source:
                                ...
                                break # Assuming we only need one set of ECHA statements per chemical

        if hazard_statements_raw:
            return "\n".join(sorted(list(set(hazard_statements_raw))))
        else:
            return f"No GHS Hazard Statements (full strings) found from ECHA for {chemical_name} (CID: {cid}) in the expected 'GHS Classification' section."
    ...
    if cas_numbers_list:
        for cas_number in cas_numbers_list:
            ...
            if h_code_results_string:
                ...
                st.info(f"No H-codes or safety information found from ECHA for CAS number {cas_number}.")
